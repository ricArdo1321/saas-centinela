...
// Helper to check if IP is in allowlist (simplified)
...
fastify.post('/v1/ata/judge', async (request, reply) => {
  ...
  fastify.log.info({ request_id, tenant_id, commands_count: commands?.length }, '⚖️ Judging actions');
  ...
  for (const cmd of commands) {
    ...
    if (!safetyCheck.safe) {
      fastify.log.warn({ request_id, cmd, reason: safetyCheck.reason }, '⛔ Command blocked by Safety Rules');
      return {
        request_id,
        result: 'fail',
    ...
    for (const ip of ipsFound) {
      if (isIpAllowed(ip, effectiveAllowlist)) {
        fastify.log.warn({ request_id, cmd, ip }, '⛔ Command blocked: Target IP is allowed/critical');
        return {
          request_id,
          result: 'fail',
  ...
  fastify.log.info({ request_id }, '✅ All commands passed validation');
...
const start = async () => {
  try {
    ...
    fastify.log.info(`Judge Agent listening at http://0.0.0.0:${PORT}`);
  } catch (err) {
