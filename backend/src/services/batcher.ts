 ...
 * Enhances digests using AI Writer reports when available.
 */
...
async function createDigestForTenant(tenantId: string): Promise<DigestResult | null> {
  ...
  // Try to enhance digest using AI Writer reports
  const aiReport = await getBestAIReportForDetections(tenantId, detectionIds);

  // Build subject and body (AI report takes precedence)
  const subject = aiReport?.subject || buildSubject(tenantId, detections.length, highestSeverity);
  const bodyText = aiReport?.body || buildBodyText(detections);

  // Create digest
  const result = await sql`
    INSERT INTO digests (
      tenant_id, window_start, window_end, severity,
      detection_count, event_count, subject, body_text, locale, ai_report_id
    ) VALUES (
      ${tenantId},
  ...
  // Link AI report to digest (if used)
  if (aiReport?.id) {
    await sql`
      UPDATE ai_reports
      SET digest_id = ${digestId}
      WHERE id = ${aiReport.id}
    `;
  }
...
async function getBestAIReportForDetections(
  tenantId: string,
  detectionIds: string[]
): Promise<{ id: string; subject: string; body: string } | null> {
  if (detectionIds.length === 0) return null;

  // Prefer the most severe/latest AI report among detections in this digest
  const reports = await sql`
    SELECT id, subject, body, severity, created_at
    FROM ai_reports
    WHERE tenant_id = ${tenantId}
      AND detection_id = ANY(${detectionIds})
...
export async function getPendingDigests(): Promise<DigestResult[]> {
  const results = await sql`
    SELECT d.id, d.tenant_id, d.detection_count, d.event_count, d.severity, d.ai_report_id
    FROM digests d
    WHERE NOT EXISTS (
  ...
  return results.map(row => ({
    ...
    aiReportId: row.ai_report_id as string | undefined
  }));
}
